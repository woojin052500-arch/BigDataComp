...
| 강우 | 기상청 AWS 시간자료 108개 CSV, 15지점 | 2021년 이후 유효 13지점. 실제 결측(행누락+NaN) 0.29~2.17%. **904 사상(2020-10, 3.7 km)·942 남구(2019-04, 2.3 km) 이설** |
| DEM | Copernicus GLO-30 (국내 DEM은 국외반출 제한으로 불가) | DSM이라 건물·수목 혼입. 픽셀 25.3 × 30.8 m |
| 도로망 | OSM walk — 무향 구간 **84,066**, 노드 58,752, 계단 **599개소 15.6 km**(OSM 피처) | 구간 중앙값 55.6 m, 20 m 미만 19.3%, **name 결측 48.5%** |
| 행정경계 | SGIS 행정동 **206개** | — |
...
`outputs/tables/` — `aws_지점연도_강우요약.csv`(108행, 기대시간수·행누락 포함), `aws_지점_결측요약.csv`(15), `aws_지점정보_부산15.csv`(15), `aws_지점이력_부산15.csv`(44),
`행정동별_관로노후도.csv`(**206행**, SGIS 전부·신뢰도 컬럼), `구군별_관로노후도.csv`(**16행**), `osm_계단_및_무명구간_집계.csv`

2026-09-07 재검토로 정정된 항목 7건은 `docs/구간1_재검토_20260907.md`, 교훈은 `docs/인수인계_20260905/05_정정이력.md` 10~16.

> **주의**: 맨홀 `설치일자`는 매설 시점이 아니라 전산 등록 시점으로 의심된다
> (12-31이 13.1%, 01-01이 10.3%, **설치일자==측량일자 59.8%**). P항 가중치 0.45가 이 변수 위에 서 있으므로
